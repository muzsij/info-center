...
import {
    buildTotalsSection, addTotalsRow, buildTaskListSection, addTaskRow,
    messageLabel, formatHM,
...
export class Redmine {
    constructor(settings, getSession, extensionPath, onEarningsChanged = () => {}) {
        this._settings = settings;
        this._getSession = getSession;
        // Called whenever monthEarnings() may have changed (a render, an error,
        // or the totals being hidden), so the earnings goal can re-evaluate.
        this._onEarningsChanged = onEarningsChanged;
    ...
    // The task lists and the monthly totals are built by separate calls so the
    // indicator can interleave other services' sections between them (the
    // ClickUp task lists sit between the Redmine tasks and the Redmine
    // monthly time).
    buildTasksMenu(menu) {
        this._menu = menu;

        this._todaySection = buildTaskListSection(
            menu, 'Redmine Issues — Today', this._iconPath);
        this._tomorrowSection = buildTaskListSection(
            menu, 'Redmine Issues — Tomorrow', this._iconPath);
    }

    buildTotalsMenu(menu) {
    ...
    _hideTotals() {
        ...
        this._item.hide();
        this._onEarningsChanged();
    }

    // This month's estimated earnings, for the earnings goal: 'off' when the
    // monthly totals are disabled or no hourly rate is set (contributes nothing),
    // 'pending' while enabled but without a successful fetch (loading or error),
    // else 'ok' with the amount.
    monthEarnings() {
        const rate = this._settings.get_double('redmine-hourly-rate');
        const configured =
            this._settings.get_string('redmine-url').trim() !== '' &&
            this._settings.get_string('redmine-api-key').trim() !== '' &&
            this._settings.get_strv('redmine-projects').length > 0;
        if (!configured || rate <= 0) {
            return {status: 'off', amount: 0};
        }
        if (!this._hoursByProject) {
            return {status: 'pending', amount: 0};
        }
        return {status: 'ok', amount: this._selectedHours(this._hoursByProject) * rate};
    }

    // Hours summed over the selected projects only.
    _selectedHours(hoursByProject) {
        return this._settings.get_strv('redmine-projects').reduce(
        ...
        if (issues.length === 0) {
            ...
            for (const issue of issues) {
                addTaskRow(
                    ...
                    baseUrl ? `${baseUrl}/issues/${issue.id}` : null
                );
            }
        }
    ...
    _updateDisplay(hoursByProject, namesByProject = {}) {
        ...
        const total = this._selectedHours(hoursByProject);
