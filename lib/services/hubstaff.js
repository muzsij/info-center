...
import {buildTotalsSection, addTotalsRow, messageLabel, formatHM} from '../shared/sections.js';
...
export class Hubstaff {
    constructor(settings, getSession, extensionPath, onEarningsChanged = () => {}) {
        ...
        // or the section being hidden), so the earnings goal can re-evaluate.
    ...
    // This month's estimated earnings, for the earnings goal: 'off' when no PAT
    // or no hourly rate is set (contributes nothing), 'pending' while enabled but
    // without a successful fetch (loading or error), else 'ok' with the amount.
    monthEarnings() {
        const rate = this._settings.get_double('hubstaff-hourly-rate');
        if (!this._currentRefreshToken() || rate <= 0) {
            return {status: 'off', amount: 0};
        }
        if (!this._secondsByProject) {
            return {status: 'pending', amount: 0};
        }
        const seconds = Object.values(this._secondsByProject)
            .reduce((sum, s) => sum + Math.max(0, s), 0);
        return {status: 'ok', amount: (seconds / 3600) * rate};
    }
