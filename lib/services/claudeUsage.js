...
// Title/tag for the model-scoped weekly limit section. The API scopes the limit
// to a model by display name ("Fable" for Fable 5); brand the known one with
// its product name, and fall back to the raw name so a future scoped model
// still gets a sensible label instead of a wrong hardcoded one.
function scopedWeeklyTitle(name, compact) {
    if (compact) {
        return `${name} - 7 day`;
    }
    return name === 'Fable' ? 'Fable 5 - Weekly' : `${name} - Weekly`;
}

// Pick the model-scoped weekly limit out of the response's generic `limits[]`
// array (kind=weekly_scoped with a model scope). Returns null when absent —
// the API may stop serving it at any time, and the section hides with it.
function findScopedWeekly(data) {
    if (!Array.isArray(data.limits)) {
        return null;
    }
    return data.limits.find(l =>
        l?.kind === 'weekly_scoped' &&
        typeof l?.percent === 'number' &&
        l?.scope?.model?.display_name) ?? null;
}
...
export class ClaudeUsage {
    ...
    _setMessage(label, fiveHour, sevenDay) {
        ...
        // The scoped weekly reading is no longer valid either.
        this._setScopedVisible(false);
    }
    ...
    _showRefreshing() {
        if (this._hasData) {
            ...
            this._scopedResetLabel.set_text('Refreshing…');
            return;
        }
    ...
    buildMenu(menu) {
        ...
        if (this._compact) {
            ...
            // Model-scoped weekly window (e.g. Fable 5), appended to the same
            // compact block. Hidden until the API actually reports one.
            const scoped = buildCompactWindow(
                compact.box, scopedWeeklyTitle('Fable', true));
            this._scopedPercent = scoped.percent;
            this._scopedProgressBar = scoped.bar;
            this._scopedProgressBg = scoped.bg;
            this._scopedResetLabel = scoped.resetLabel;
            this._scopedTitleLabel = scoped.tagLabel;
            this._scopedActors = [scoped.barRow, scoped.resetRow];
            this._setScopedVisible(false);
        ...
        // Model-scoped weekly section (e.g. Fable 5), with its own separator.
        // Hidden until the API actually reports one.
        const scopedSeparator = new PopupMenu.PopupSeparatorMenuItem();
        scopedSeparator.add_style_class_name('info-center-separator');
        menu.addMenuItem(scopedSeparator);

        const scoped = buildUsageSection(
            menu, scopedWeeklyTitle('Fable', false), this._iconPath);
        this._scopedPercent = scoped.percent;
        this._scopedProgressBar = scoped.bar;
        this._scopedProgressBg = scoped.bg;
        this._scopedResetLabel = scoped.resetLabel;
        this._scopedTitleLabel = scoped.titleLabel;
        this._scopedActors = [scopedSeparator, scoped.item];
        this._setScopedVisible(false);
    ...
    // Show/hide the model-scoped weekly section: separator + section item in
    // the full layout, the two extra rows of the compact block in compact.
    _setScopedVisible(visible) {
        for (const actor of this._scopedActors) {
            if (visible) {
                actor.show();
            } else {
                actor.hide();
    ...
    _updateDisplay(data) {
        ...
        // Model-scoped weekly limit (e.g. Fable 5). Only shown while the API
        // reports one — the section disappears if the field goes away.
        const scoped = findScopedWeekly(data);
        if (scoped) {
            this._scopedTitleLabel.set_text(scopedWeeklyTitle(
                scoped.scope.model.display_name, this._compact));
            this._scopedPercent.set_text(`${scoped.percent.toFixed(1)}%`);
            updateProgressBar(
                this._scopedProgressBar, this._scopedProgressBg, scoped.percent);
            this._scopedResetLabel.set_text(scoped.resets_at
                ? `Resets in ${formatResetCountdown(scoped.resets_at)}`
                : '—');
            this._setScopedVisible(true);
        } else {
            this._setScopedVisible(false);
        }
    }
    ...
    _reapplyBars() {
        ...
        this._reapplyId = GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
            ...
            applyBarWidth(this._scopedProgressBar, this._scopedProgressBg);
            return GLib.SOURCE_REMOVE;
        });
    }
}
