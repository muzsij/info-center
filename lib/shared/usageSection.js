...
// The width of `actor`'s last allocation. Unlike get_width(), this doesn't fall
// back to the preferred width once a child resize has queued a relayout — for a
// bar track that preferred width is just the fill's own width, so reading it
// right after a set_width (or while the menu is closed and nothing reallocates)
// would size or position things against the wrong width.
export function allocatedWidth(actor) {
    return actor.get_allocation_box().get_width();
}

const USAGE_LEVELS = ['usage-low', 'usage-medium', 'usage-high', 'usage-critical'];

// The single source of truth for the usage color thresholds (65/80/90), shared
// by the dropdown bars and the panel bar so both change color at the same
// readings. The matching colors live in stylesheet.css.
function usageLevel(usage) {
    return usage >= 90 ? 'usage-critical'
        : usage >= 80 ? 'usage-high'
        : usage >= 65 ? 'usage-medium'
        : 'usage-low';
}

// Swap in the usage-* class for this reading, dropping the other three.
function applyUsageLevel(bar, usage) {
    const level = usageLevel(usage);
    for (const cls of USAGE_LEVELS) {
        if (cls === level) {
...
export function updatePanelProgressBar(panelBar, usage) {
    ...
    applyUsageLevel(panelBar, usage);
}
