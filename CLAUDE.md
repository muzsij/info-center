...
Both feature modules receive the `Soup.Session` through a `getSession` getter (`() => this._session`) rather than a stored reference, so when the indicator recreates the session on a proxy change the modules pick up the new session on their next request. They are passed `settings` directly and read their own keys. `Redmine` creates no timers or signals of its own, but owns a `Gio.Cancellable` for its in-flight fetches and exposes `destroy()` to cancel it. `ClaudeUsage` owns a short-lived auth-retry timer and a `Gio.Cancellable` for its in-flight async I/O (see below) and exposes `destroy()` to tear both down; the indicator's `destroy()` calls `this._claude.destroy()` and `this._redmine.destroy()`. All menu items are owned by `this.menu` (destroyed by `super.destroy()`).
...
- **Lifecycle / leaks:** the indicator's `destroy()` must stop the refresh timer, remove the debounce timer, call `this._claude.destroy()` and `this._redmine.destroy()`, abort the Soup session, and disconnect the settings signal. Both feature modules own a `Gio.Cancellable` (and `ClaudeUsage` a retry timer) torn down in their `destroy()` — if you add another timer, signal, or session inside `claudeUsage.js` / `redmine.js`, extend that teardown and ensure the indicator calls it, or it leaks across enable/disable cycles (a common reason extensions get rejected from extensions.gnome.org).
